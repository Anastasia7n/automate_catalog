...
BASEPATH_SUPPLIER_TEXT_DES = os.path.expanduser('~') + '/supplier-data/description/'
...
for text_file in list_text_files:
    with open(BASEPATH_SUPPLIER_TEXT_DES + text_file, 'r') as f:
        text_data.append([line.strip() for line in f.readline()])
        f.close()

if __name__ == "__main__":
    summary = process_data(text_data)

# Ganerate the Paragraph that contains the necessary summary
